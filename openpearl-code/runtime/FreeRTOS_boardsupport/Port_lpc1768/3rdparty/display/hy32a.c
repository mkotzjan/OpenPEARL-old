...
#define FIODIR DIR
#define FIOPIN PIN
#define INLINE32A /* __attribute__((always_inline))  */
#define MDATA   (*(uint8_t*)(&(LPC_GPIO2->PIN)))
#define MDIR   (*(uint8_t*)(&(LPC_GPIO2->DIR)))
...
*******************************************************************************/
static 
INLINE32A void LCD_Send (uint16_t byte) 
{
	...
	LPC_GPIO2->FIOCLR =  0x0ff;
        LPC_GPIO2->FIOPIN = (byte & 0x0ff);          /* Write D0..D7 */
	LCD_LE(1)                         
	LCD_LE(0)							/* latch D0..D7	*/
	LPC_GPIO2->FIOCLR &=  0x0ff;
	LPC_GPIO2->FIOPIN =  (byte >> 8) & 0x0ff;     /* Write D8..D15 */
}
...
{
//    for (xcount=0; xcount<10*count; xcount++);
...
* Description    : LCD read byte
* Input          : - byte: byte to be read
* Output         : None
* Return         : read byte
* Attention		 : None
*******************************************************************************/
static INLINE32A uint16_t LCD_Read (void) 
{
	uint16_t value;
	...
	wait_delay(30);							   /* delay some times */
	value = (value << 8) | (0x0ff & LPC_GPIO2->FIOPIN); /* Read D0..D7 */
	LCD_DIR(1);
	return  value;
}
...
* Description    : set write index for LCD
* Input          : - index
* Output         : None
* Return         : None
* Attention		 : None
*******************************************************************************/
static INLINE32A void LCD_WriteIndex(uint16_t index)
{
	LCD_CS(0);
	LCD_RS(0);
	LCD_RD(1);
...
* Description    : LCD write data byte
* Input          : - index
* Output         : None
* Return         : None
* Attention		 : None
*******************************************************************************/
static INLINE32A void LCD_WriteData(uint16_t data)
{				
	LCD_CS(0);
	LCD_RS(1);   
	LCD_Send( data );
...
* Attention		 : None
*******************************************************************************/
static INLINE32A uint16_t LCD_ReadData(void)
{ 
	uint16_t value;
...
* Attention		 : None
*******************************************************************************/
static INLINE32A void LCD_WriteReg(uint16_t LCD_Reg,uint16_t LCD_RegValue)
{ 
...
* Attention		 : None
*******************************************************************************/
static INLINE32A  uint16_t LCD_ReadReg(uint16_t LCD_Reg)
{
	uint16_t LCD_RAM;
