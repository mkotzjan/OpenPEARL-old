 ...
 * pvPortMalloc returns ether a  previously freed memory segment of the EXACT size
 * (similar to heap_2.c), or a memory segment from the static allocated heap space
 * (like in heap_1), or an error if the heap space is exhausted.
 *
...
BlockLink_t * 	pxBlock;
	...
	vTaskSuspendAll();
	{
		...
		pxBlock = xStart->pxNextFreeBlock;  /* the 'zero' block need not to be checked */
		pvReturn = NULL;
...
//printf("prvMalloc: reuse of block at %p size=%d\n", pvReturn, pxBlock->xBlockSize);
		...
		/* If we found NO EXACTLY fitting block allocate from the static heap storage */
		if( pvReturn == NULL )
		{
			...
			if( ( ( xNextFreeByte + xWantedSize +heapSTRUCT_SIZE ) < configADJUSTED_HEAP_SIZE ) &&
				( ( xNextFreeByte + xWantedSize +heapSTRUCT_SIZE ) > xNextFreeByte )	)/* Check for overflow. */
			{
				/* Return the next free byte then increment the index past this block. 
                 * add the linked list elements just before the returned pointer      
                 *  			+-----------------------------------+ 
