...
The low level device drivers depend on the specific target hardware plattform.
They are selected using {\em Configuration Items} in the OpenPEARL system part.
These modules will register themself in the diskio-module, which delegates 
the calls from tghe fatfs-modukle (ff.c) to the device drivers.
.
...
\section{To be done}
  \begin{itemize}
  \item provide device drivers for the standard devices (SD, Ethernet)
