...
\item Attribute \index{TFU@\textbf{TFU}}
...
\begin{added}
Details about the behavior of TFU:
\begin{itemize}
\item If the system dation requires transfer units, this must be specified
in the system dation specification. 
\begin{discuss}
	wie?
	
	SPC ... DATION ... TFU(64) ? 
	Und die 64 wird vom IMC mit der Gr"o"se der 
	Systemdation abgeglichen?
	
	oder wird die Transfergroesse von der Systemdation uebernommen?
	\end{discuss}
\item If a user dation is declared with TFU, the record length is defined by
  \begin{itemize}
   \item the position of the end-od-record marker (e.g. NL) for ALPHIC dations
      with assecc attribute FORWARD
  ...
  \end{itemize}
\item If the system dation does not require TFU, the behavior is simulated
  by the user dation.
\item If the system dation defines a transfer unit size,
   the record length of the 
   user dation must not exceed this length.
   \begin{discuss}
   	wie?? siehe oben
   \end{discuss}
\item On output: If the record length of the system dation is larger than
   the record length of the user dation, the remaining elements
    are padded with zeros or spaces according the dation type.
\item On input: If the record length of the system dation is larger than
   the record length of the user dation, the remaining elements must be equal
   to the padding element.
   If the input record is shorter, the remaining elements are padded
   with  zeros or spaces according the dation type.
\item On output, the TFU buffer is transfered to the device:
  \begin{itemize}
   \item at the end of the output statement
   \item if a positioning statement leaves the current record
  \end{itemize}
\item On input, a new TFU record is read from the device:
  \begin{itemize}
   \item with the first data input format element in the input statement
   \item with the first data input format element in the input statement
   after a positioning statement which leaves the current record
  \end{itemize}
\end{itemize}
\end{added}
