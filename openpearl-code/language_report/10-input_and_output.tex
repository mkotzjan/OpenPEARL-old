...
On a disk memory drive with system name \removedtext{PSP31}
\acceptedtext{Disc}  and user name Disk, a file
...
On a disk drive with system name \removedtext{PSP31}\addedtext{Disc}
...
\label{Example}
{\bf PROBLEM};\\
\x {\bf SPC} Disk {\bf DATION INOUT \acceptedtext{SYSTEM} ALL};\\
...
Muss noch in die Liste der OpenParameter
\end{discuss}
\end{itemize}
...
\begin{discuss}
das nur, wenn Close-Parameter bei OPEN weiterhin zulaessig bleiben.
Sonst die beiden Attribute hier erklaeren.
\end{discuss}
...
Frage: wieso nicht auch IDF identisch?
...
\x Printer: DRUA;\\
\removedtext{\x Disk: PSP31;\\}
\acceptedtext{\x Disk: Disc('/home' ,10);\\}
\x ... \\
{\bf PROBLEM};\\
\x {\bf SPC} Printer {\bf DATION OUT \acceptedtext{SYSTEM} ALPHIC};\\
...
\x {\bf SPC} File {\bf DATION INOUT \acceptedtext{SYSTEM} ALL};\\
...
\end{front}
\begin{discuss}
EOF bei RealtivePosition  ergaenzen?
\end{discuss}
\begin{grammar}
...
\end{grammar}
\begin{discuss}
separation between absolute and relative pos is better
\end{discuss}
...
\begin{discuss}
\item EOF\\
refers to the end of the dation. If used in conjunction with fixed topology
...
\begin{front}
FixedFormat ::=\\
\x {\bf F} (FieldWidth [ , DecimalPositions \removedtext{[ , ScaleFactor ]} ] )
...
\acceptedtext{(E) or three (E3)} digits.
...
%     hold analogously.
\item If w $<=$ 0, or d $<$ 0, or s $<$ \removedtext{0}\addedtext{1}, no assignment happens and  
...
\end{tabular}
\begin{discuss}
wieso keine Rundung bei 27.13E+2 ? muesste m.E 27.14E+02 heissen
\end{discuss}
\end{accepted}
...
\end{removed}
\item {\bf Input}\\
If w $<=$ 0 \removedtext{or d $<$ 0}, no data is read and the DurationFormatSignal
   is induced.

\begin{accepted}
A field of length w is read in, which must contain a duration in a
permitted representation \removedtext{(see 1)}. Preceding and following spaces are
ignored.
...
{\bf PROBLEM};\\
\x {\bf SPC} motor {\bf DATION OUT \addedtext{SYSTEM} \removedtext{BASIC}
    \addedtext{BIT(4)} };\\
\x \addedtext{{\bf DCL} m {\bf DATION OUT BASIC BIT(4) CREATED} ( motor );}\\
\x {\bf DCL} on {\bf INV BIT}(4) {\bf INIT}('1010'B1);

\x {\bf SEND} on {\bf TO} m\removedtext{otor};
...
\addedtext{ and micro controllers} --- existing I/O
...
\begin{discuss}
das lassen wir doch auch lieber in einem separaten Dokument, oder?
\end{discuss}
