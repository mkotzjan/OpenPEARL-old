...
\usepackage{tikz}
\usepackage{geometry}
\geometry{paperheight=14.6cm, paperwidth=17cm, margin={2mm,2mm}}
...
\newcommand*\circled[1]{\tikz[baseline=(char.base)]{
  ...
  \begin{tikzpicture}[
    ...
    \node[block, fill=yellow!20] (sprachumsetzer) at (5,10) {\shortstack{\circled{1}\\ compiler}};
    \node[file,below of=sprachumsetzer,yshift=0.5cm] (problemcc) {\shortstack{C++ code\\of Problem\\part}};
    \node[file,below of=prl,yshift=0.5cm] (modulxml) {\shortstack{XML description
                                                            \\of \\
                                                             export+import}};
    ...
    \node[file, yshift=-0.5cm,left of=modulxml] (hardwarexml) {\shortstack{XML description \\of predefined\\system names}};
    ...
    \node[blockold, right of=systemcc,xshift=1.5cm,yshift=-1cm] (gcc)
...
{\shortstack{platform specific \\ runtime library}};
   \node[blockold, above of=runtimelib] (gcc1){gcc tool chain};
   \node[block, above of=gcc1,fill=yellow!20] (runtimepackage)
{\shortstack{\circled{2} \\C++ code of \\ platform specific \\ runtime system}};
    ...
    \node[file, below of=gcc,yshift=0.5cm] (pearlapp) {\shortstack{PEARL\\application}};
    \draw[->] (gcc) -- (pearlapp);
  \end{tikzpicture}
\end{document}